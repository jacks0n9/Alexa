...
gst_pulsering_stream_underflow_cb (pa_stream * s, void *userdata)
{
  ...
  // Post a warning message on the bus
  GST_ELEMENT_WARNING(psink, LAB126, UNDERFLOW, ("Got Underflow"), (NULL));
}
...
gst_pulsering_stream_started_cb (pa_stream * p, void *userdata)
{
  ...
  // Post an info message on the bus
  GST_ELEMENT_INFO(psink, LAB126, PA_STREAM_START, ("stream started"), (NULL));
}
    ...
    GstAudioRingBufferSpec * spec)
{
  ...
  /* try to connect now */
  GST_LOG_OBJECT (psink, "connect for playback to device NULL");

  if (pa_stream_connect_playback (pbuf->stream, NULL,
    ...
    GstPulseRingBuffer * pbuf, pa_format_info * format)
{
  ...
  if (pa_stream_connect_playback (stream, NULL, NULL, flags, NULL,
          NULL) < 0)
    goto error;
