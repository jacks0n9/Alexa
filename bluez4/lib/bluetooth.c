...
/* Returns an obfuscated form of the BT address
 * Assumes original BD ADDR to be of the format AA:BB:CC:DD:EE:FF
 * Curated form of the BD ADDR will be XX:XX:XX:XX:EE:FF
 * This is only done for the purpose of logging.
 */
int str2obfstr(const char *str, char *obf_str)
{
	if(!str || !obf_str)
		return -1;

	if(strlen(str) != BDADDR_SIZE)
		return -1;

	// Create the curated string
	strncpy(obf_str, DEFAULT_OBFUSCATED_ADDR, strlen(str));
	obf_str[12] = str[12];
	obf_str[13] = str[13];
	obf_str[15] = str[15];
	obf_str[16] = str[16];
