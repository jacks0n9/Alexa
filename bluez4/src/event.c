...
int btd_event_request_pin(bdaddr_t *sba, bdaddr_t *dba, gboolean secure)
{
	struct btd_adapter *adapter;
	struct btd_device *device;
	char pin[MAX_PIN_LENGTH + 1], addr[18], obf_addr[18];
	ssize_t pinlen;
	...
	DBG("PIN code requested by remote device %s", obf_addr);

	memset(pin, 0, sizeof(pin));
	pinlen = btd_adapter_get_pin(adapter, device, pin, sizeof(pin), &display);
