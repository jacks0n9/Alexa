				...
				const gchar *address, uint8_t bdaddr_type)
{
	...
	bdaddr_t src;
	char srcaddr[18], alias[MAX_NAME_LENGTH + 1], obf_addr[18];
	...
	str2obfstr(address_up, obf_addr);
	DBG("Creating device %s/dev_%s", adapter_path, obf_addr);
...
void device_remove(struct btd_device *device, gboolean remove_stored)
{
	...
	DBG("Removing device %s", obf_addr);
...
void device_probe_drivers(struct btd_device *device, GSList *profiles)
{
	...
	if (device->blocked) {
		DBG("Skipping drivers for blocked device %s", obf_addr);
		goto add_uuids;
	}

	DBG("Probing drivers for %s", obf_addr);
...
static void device_remove_drivers(struct btd_device *device, GSList *uuids)
{
	...
	GSList *list, *next;
	char srcaddr[18], dstaddr[18], obf_dstaddr[18];
	bdaddr_t src;
	sdp_list_t *records;
	...
	ba2str(&src, srcaddr);
	ba2str(&device->bdaddr, dstaddr);
	str2obfstr(dstaddr, obf_dstaddr);

	records = read_records(&src, &device->bdaddr);

	DBG("Removing drivers for %s", obf_dstaddr);
...
static void search_cb(sdp_list_t *recs, int err, gpointer user_data)
{
	...
	if (!req->profiles_added && !req->profiles_removed) {
		DBG("%s: No service update", obf_addr);
		goto send_reply;
	}
...
static gboolean att_connect(gpointer user_data)
{
	...
	DBG("Connection attempt to: %s", obf_addr);
...
void device_set_auto_connect(struct btd_device *device, gboolean enable)
{
	...
	DBG("%s auto connect: %d", obf_addr, enable);
						...
						uint8_t capability)
{
	...
	ba2str(&device->bdaddr, addr);
	str2obfstr(addr, obf_addr);
	DBG("Requesting bonding for %s", obf_addr);
...
static void create_bond_req_exit(DBusConnection *conn, void *user_data)
{
	...
	ba2str(&device->bdaddr, addr);
	str2obfstr(addr, obf_addr);
	DBG("%s: requestor exited before bonding was completed", obf_addr);
...
void device_bonding_complete(struct btd_device *device, uint8_t status)
{
	...
	/* If we're already paired, emit the "Paired" property changed always*/
	if (device->paired) {
		DBusConnection *conn = get_dbus_connection();
		emit_property_changed(conn, device->path, DEVICE_INTERFACE, "Paired",
				DBUS_TYPE_BOOLEAN, &device->paired);
		return;
	}
...
void device_cancel_bonding(struct btd_device *device, uint8_t status)
{
	...
	ba2str(&device->bdaddr, addr);
        str2obfstr(addr, obf_addr);
	DBG("Canceling bonding request for %s", obf_addr);
					...
					void *data, gboolean secure, void *cb)
{
	...
	ba2str(&device->bdaddr, addr);
	str2obfstr(addr, obf_addr);
	DBG("Requesting agent authentication for %s", obf_addr);

	if (device->authr) {
		error("Authentication already requested for %s", obf_addr);
		return -EALREADY;
	}
...
void device_cancel_authentication(struct btd_device *device, gboolean aborted)
{
	...
	ba2str(&device->bdaddr, addr);
	str2obfstr(addr, obf_addr);
	DBG("Canceling authentication request for %s", obf_addr);
