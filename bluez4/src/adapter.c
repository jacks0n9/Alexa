...
static void load_connections(struct btd_adapter *adapter)
{
	...
	for (l = conns; l != NULL; l = g_slist_next(l)) {
		...
		str2obfstr(address, obf_addr);
		DBG("Adding existing connection to %s", obf_addr);
...
void btd_adapter_start(struct btd_adapter *adapter)
{
	...
	info("Adapter for device %s has been enabled", obf_addr);
}
...
struct btd_adapter *adapter_create(DBusConnection *conn, int id)
{
	...
	// Register a PIN code cb to this adapter
	btd_adapter_register_pin_cb(adapter, speaker_pin_create_request_cb);
...
void adapter_remove(struct btd_adapter *adapter)
{
	...
	// Unregister callbacks bound to this adapter
	btd_adapter_unregister_pin_cb(adapter, speaker_pin_create_request_cb);

	DBG("Removing adapter %s", obf_addr);
						...
						struct btd_device *device)
{
	...
	if (device_is_temporary(device)) {
		...
		ba2str(&adapter->bdaddr, addr);
		str2obfstr(addr, obf_addr);
		DBG("Removing temporary device %s", obf_addr);
...
ssize_t speaker_pin_create_request_cb(struct btd_adapter *adapter, struct btd_device *dev,
					char *pin_buf, ssize_t pin_len, gboolean *display)
{
	DBG("Creating PIN 0000 as default code");

	//Ensure pin code length requested doesn't exceed max supported length + '\0' char
	if(pin_len > (MAX_PIN_LENGTH + 1))
		return -EINVAL;

	memset(pin_buf, 0, pin_len);
	pin_buf[0] = 0x30;
	pin_buf[1] = 0x30;
	pin_buf[2] = 0x30;
	pin_buf[3] = 0x30;
					...
					char *pin_buf, ssize_t pin_len, gboolean *display)
{
	...
	for (l = adapter->pin_callbacks; l != NULL; l = g_slist_next(l)) {
		cb = l->data;
		ret = cb(adapter, dev, pin_buf, pin_len, display);
		if (ret > 0)
			return ret;
	}
...
int btd_adapter_read_voice_setting(struct btd_adapter *adapter, uint16_t *vs)
{
	return adapter_ops->read_voice_setting(adapter->dev_id, vs);
}

int btd_adapter_write_voice_setting(struct btd_adapter *adapter, uint16_t vs)
{
	return adapter_ops->write_voice_setting(adapter->dev_id, vs);
}
