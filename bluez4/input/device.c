				...
				void *user_data)
{
	...
	info("Input: disconnect device %s", obf_addr);
...
static void device_unregister(void *data)
{
	...
	ba2str(&idev->dst, addr);
	str2obfstr(addr, obf_addr);
	DBG("Unregistered interface %s for device %s", INPUT_DEVICE_INTERFACE,
								obf_addr);
				...
				const uint32_t handle, gboolean disable_sdp)
{
	...
	struct input_device *idev;
	char name[249], src_addr[18], dst_addr[18], obf_addr[18];
	...
	if (g_dbus_register_interface(conn, idev->path, INPUT_DEVICE_INTERFACE,
		...
		error("Failed to register interface %s on addr %s",
			INPUT_DEVICE_INTERFACE, obf_addr);
		input_device_free(idev);
		return NULL;
	}

	DBG("Registered interface %s on addr %s",
			INPUT_DEVICE_INTERFACE, obf_addr);
