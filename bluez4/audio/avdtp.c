...
#define AVDTP_SINK_RS_WILDCARD 0xffff
#define AVDTP_SLAVE_MODE 1
#define AVDTP_SWITCH_ROLE_TIMEOUT 10000
...
// holds a list of vids pids to role switch when a2dp is sink
static uint32_t *vids_pids;
static gsize vids_pids_len;
...
static void connection_lost(struct avdtp *session, int err)
{
	...
	DBG("Disconnected from %s", obf_addr);
							...
							void *buf, int size)
{
	...
	if (session->state == AVDTP_SESSION_STATE_DISCONNECTING) {
		uint8_t err = AVDTP_BAD_STATE;
		error("Rejecting AVDTP_DISCOVER due to bad state %d", session->state);
		return avdtp_send(session, transaction, AVDTP_MSG_TYPE_REJECT,
					AVDTP_DISCOVER, &err, sizeof(err));
	}
				...
				struct seid_req *req, unsigned int size)
{
	...
	avdtp_set_state(session, AVDTP_SESSION_STATE_DISCONNECTING);
...
// true if the device matches the audio.conf filter for
// vid pids to to role switch for a2dp sink
static gboolean find_vid_pid(const uint32_t vid_pid)
{
	int length;
	int i;
	...
	for (i = 0; i < vids_pids_len; i++) {
		uint32_t v_p = vids_pids[i];
		// 3 filters:
		// 1. match any vid pid if v_p = 0xffff_ffff
		// 2. match the vid and have wildcard for pid
		// 3. match the vid and pid
		if ((v_p ==(AVDTP_SINK_RS_WILDCARD << 16 | AVDTP_SINK_RS_WILDCARD)) ||
			(((v_p & 0xffff) == AVDTP_SINK_RS_WILDCARD) &&
			((v_p >> 16) == (vid_pid >> 16))) ||
			(v_p == vid_pid))
			return TRUE;
	}
...
static void avdtp_confirm_cb(GIOChannel *chan, gpointer data)
{
	...
	str2obfstr(address, obf_addr);
	DBG("AVDTP: incoming connect from %s", obf_addr);
...
int avdtp_start(struct avdtp *session, struct avdtp_stream *stream)
{
	 ...
	 *
	 *  Note: Not all speakers do this even though it is recomended. We will
	 *  always send the START even if the RD opened the stream
	 */
...
// sends hci role switch cmd
void avdtp_hci_cmd_switch_role_slave(const bdaddr_t *src_ba, const bdaddr_t *dst_ba)
{
	bdaddr_t bdaddr;
	int opt, dd;
	char src_addr[18];
	...
	ba2str(src_ba, src_addr);
	ba2str(dst_ba, dst_addr);
	...
	hci_switch_role(dd, dst_ba, AVDTP_SLAVE_MODE, AVDTP_SWITCH_ROLE_TIMEOUT);

	hci_close_dev(dd);
}

// role switch if the echo is a2dp sink and the vid pid of the source matches
// the entry 'SlaveSink' in audio.conf
void avdtp_switch_role(struct avdtp *session)
{
	struct btd_adapter *adapter;
	struct btd_device *device;
	...
	if (find_vid_pid(vendor<<16 | product))
		avdtp_hci_cmd_switch_role_slave(&session->server->src, &session->dst);
}
...
int avdtp_init(const bdaddr_t *src, GKeyFile *config, uint16_t *version)
{
	...
	list = g_key_file_get_string_list(config, "General", "SlaveSink",
						&vids_pids_len, NULL);
	if (vids_pids_len) {
		g_free(vids_pids);
		vids_pids = g_malloc(vids_pids_len*sizeof(uint32_t));
		for (i = 0; list && list[i] != NULL; i++)
			vids_pids[i] = (uint32_t) strtoul(list[i], NULL, 16);
	}
	g_strfreev(list);
