...
static void sco_server_cb(GIOChannel *chan, GError *err, gpointer data)
{
	...
	if (device->voice) {
		if (voice_get_state(device) != VOICE_STATE_PLAY_PENDING) {
			DBG("Refusing SCO from non-connected Voice device");
			goto drop;
		}

		if (voice_connect_sco(device, chan) >= 0)
			handled = TRUE;
	}
	...
	DBG("Accepted SCO connection from %s", obf_addr);
