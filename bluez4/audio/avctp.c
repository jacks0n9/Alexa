...
{
	struct audio_device *dev;
	char address[18], name[248 + 1], obf_addr[18];
	...
	if (session->uinput < 0)
		error("AVRCP: failed to init uinput for %s", obf_addr);
	else
		DBG("AVRCP: uinput initialized for %s", obf_addr);
}
...
static void avctp_connect_cb(GIOChannel *chan, GError *err, gpointer data)
{
	...
	str2obfstr(address, obf_addr);
	DBG("AVCTP: connected to %s", obf_addr);
...
static void avctp_confirm_cb(GIOChannel *chan, gpointer data)
{
	...
	str2obfstr(address, obf_addr);
	DBG("AVCTP: incoming connect from %s", obf_addr);
	...
	if (session->io) {
		error("Ignoring unexpected connect from %s", address);
		return;
	}
