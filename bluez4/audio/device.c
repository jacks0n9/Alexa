					...
					const bdaddr_t *dst)
{
	...
	if (!g_dbus_register_interface(dev->conn, dev->path,
					...
					dev, NULL)) {
		error("Unable to register %s for device %s", AUDIO_INTERFACE,
								obf_addr);
		device_free(dev);
		return NULL;
	}

	DBG("Registered interface %s for device %s", AUDIO_INTERFACE,
								obf_addr);
						...
						const char *interface)
{
	if (!interface) {
		...
		return TRUE;
	else if (!strcmp(interface, AUDIO_VOICE_INTERFACE) && dev->voice &&
				 voice_is_active(dev))
		return TRUE;
