...
static void media_endpoint_destroy(struct media_endpoint *endpoint)
{
	...
	if (endpoint->vox_watch)
		voice_remove_state_cb(endpoint->vox_watch);
...
static void voice_setconf_cb(struct media_endpoint *endpoint, void *ret,
			     int size, void *user_data)
{
	struct audio_device *dev = user_data;
...
static void voice_state_changed(struct audio_device *dev,
				...
				void *user_data)
{
	...
	case VOICE_STATE_CONNECTING:
		set_configuration(endpoint, dev, NULL, 0, voice_setconf_cb,
								dev, NULL);
		break;
...
static gboolean endpoint_init_voice(struct media_endpoint *endpoint, int *err)
{
	GSList *list;
	GSList *l;

	endpoint->vox_watch = voice_add_state_cb(voice_state_changed,
						 endpoint);
	list = manager_find_devices(NULL, &endpoint->adapter->src, BDADDR_ANY,
						AUDIO_VOICE_INTERFACE, TRUE);

	for (l = list; l != NULL; l = l->next) {
		struct audio_device *dev = l->data;

		set_configuration(endpoint, dev, NULL, 0,
				  voice_setconf_cb, dev, NULL);
	}
						...
						int *err)
{
	...
	if (strcasecmp(uuid, A2DP_SOURCE_UUID) == 0)
		...
		succeeded = endpoint_init_voice(endpoint, err);
	else {
		succeeded = FALSE;
