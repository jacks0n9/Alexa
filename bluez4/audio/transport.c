...
{
	struct audio_device *dev = transport->device;
	...
	if (g_strstr_len(accesstype, -1, "r") != NULL) {
		...
		DBG("Transport %s: read lock released", obf_addr);
	}

	if (g_strstr_len(accesstype, -1, "w") != NULL) {
		transport->write_lock = FALSE;
		DBG("Transport %s: write lock released", obf_addr);
	}
						...
						struct media_owner *owner)
{
	...
	ba2str(&dev->dst, addr);
	str2obfstr(addr, obf_addr);
	DBG("Transport %s Owner %s", obf_addr, owner->name);
							...
							const char *accesstype)
{
	...
	if (read_lock) {
		...
		DBG("Transport %s: read lock acquired", obf_addr);
	}

	if (write_lock) {
		transport->write_lock = write_lock;
		DBG("Transport %s: write lock acquired", obf_addr);
	}
...
static void voice_resume_complete(struct audio_device *dev, GError *err,
				  void *user_data)
{
	struct media_owner *owner = user_data;
	struct media_request *req = owner->pending;
...
static guint resume_voice(struct media_transport *transport,
			  struct media_owner *owner)
{
	struct audio_device *device = transport->device;
	...
	transport->in_use = voice_lock(device, VOICE_LOCK_READ |
				       VOICE_LOCK_WRITE);
	...
	return voice_request_stream(device, voice_resume_complete, owner);
}

static gboolean voice_suspend_complete(gpointer user_data)
{
	struct media_owner *owner = user_data;
	struct media_transport *transport = owner->transport;
	struct audio_device *device = transport->device;
	...
	voice_unlock(device, VOICE_LOCK_READ | VOICE_LOCK_WRITE);
	transport->in_use = FALSE;
	media_transport_remove(transport, owner);
	return FALSE;
}

static guint suspend_voice(struct media_transport *transport,
			   struct media_owner *owner)
{
	struct audio_device *device = transport->device;
	static int id = 1;
	...
	voice_suspend_stream(device);
	g_idle_add(voice_suspend_complete, owner);
	return id++;
}

static void cancel_voice(struct media_transport *transport,
			 guint id)
{
	DBG("");
	voice_cancel_stream(transport->device, id);
}

static void get_properties_voice(struct media_transport *transport,
				 DBusMessageIter *dict)
{
	/* None */
	DBG("");
...
static int set_property_voice(struct media_transport *transport,
		       const char *property,
		       DBusMessageIter *value)
{
	DBG("");
						...
						size_t size)
{
	...
	} else if (strcasecmp(uuid, VOICE_UPLOAD_UUID) == 0) {
		transport->resume = resume_voice;
		transport->suspend = suspend_voice;
		transport->cancel = cancel_voice;
		transport->get_properties = get_properties_voice;
		transport->set_property = set_property_voice;
	} else
		goto fail;
	...
	if (g_dbus_register_interface(transport->conn, transport->path,
		...
		error("Could not register transport");
		goto fail;
	}
