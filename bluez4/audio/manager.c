...
gboolean server_is_enabled_uuid128(bdaddr_t *src, const char *uuidstr)
{
	if (!strcmp(uuidstr, VOICE_UPLOAD_UUID) || !strcmp(uuidstr, VOICE_SOURCE_UUID)) {
...
/**
 * Handle the custom "Voice" profile
 *
 * The handle_uuid() function relies heavily on UUID16 values, which
 * are (mostly) reserved for official Bluetooth-SIG-recognized
 * protocols. If the UUID does not map to UUID16, then this function
 * will handle it.
 */
static void handle_uuid_custom(const char *uuidstr, struct audio_device *device)
{
	/* This code is structured with the assumption that it is only
	 * handling Voice UUID's.
	 */
	if (strcmp(uuidstr, VOICE_SOURCE_UUID) != 0
	    && strcmp(uuidstr, VOICE_UPLOAD_UUID) != 0) {
		error("Unrecognized UUID");
		return;
	}

	if (!enabled.voice) {
		error("Voice server not enabled in config");
		return;
	}

	if (device->voice)
		voice_update(device, uuidstr);
	else
		device->voice = voice_init(device, uuidstr);
}
...
static void handle_uuid(const char *uuidstr, struct audio_device *device)
{
	...
	if (bt_string2uuid(&uuid, uuidstr) < 0) {
		error("Not detected as an UUID-128");
		return;
	}

	if (!sdp_uuid128_to_uuid(&uuid) && uuid.type != SDP_UUID16) {
		handle_uuid_custom(uuidstr, device);
	...
	case AUDIO_SINK_SVCLASS_ID:
		...
		if ((device->sink == NULL) && (device->source == NULL))
...
#ifdef ENABLE_AVCTP_AUTO_CONNECT
		if (device->sink && sink_is_active(device))
			avrcp_connect(device);
#endif
		break;
	default:
		DBG("Unrecognized UUID");
		break;
	}
}
...
static sdp_record_t *voice_up_record(uint8_t ch)
{
	sdp_list_t *svclass_id, *apseq, *root;
	bt_uuid_t voice_up_uuid_tmp;
	...
	bt_string_to_uuid(&voice_up_uuid_tmp, VOICE_UPLOAD_UUID);
	sdp_uuid128_create(&svclass_uuid, &voice_up_uuid_tmp.value.u128);
	svclass_id = sdp_list_append(0, &svclass_uuid);
	sdp_set_service_classes(record, svclass_id);
	...
	sdp_set_info_attr(record, "Voice Upload Server", 0, 0);
...
static void voice_auth_cb(DBusError *derr, void *user_data)
{
	struct audio_device *device = user_data;
	GError *err = NULL;
	GIOChannel *io;

	if (device->voice_preauth_id) {
		g_source_remove(device->voice_preauth_id);
		device->voice_preauth_id = 0;
	}
	...
	io = voice_get_rfcomm(device);

	if (!bt_io_accept(io, voice_connect_cb, device, NULL, &err)) {
		error("bt_io_accept: %s", err->message);
		g_error_free(err);
		voice_set_state(device, VOICE_STATE_DISCONNECTED);
		return;
...
static gboolean voice_preauth_cb(GIOChannel *chan, GIOCondition cond,
							gpointer user_data)
{
	struct audio_device *device = user_data;

	DBG("Voice disconnected during authorization");

	audio_device_cancel_authorization(device, voice_auth_cb, device);
...
static void gateway_auth_cb(DBusError *derr, void *user_data)
{
	...
	if (derr && dbus_error_is_set(derr)) {
		...
		char ag_address[18], obf_ag_address[18];

		ba2str(&device->dst, ag_address);
		str2obfstr(ag_address, obf_ag_address);
		DBG("Accepted AG connection from %s",
			obf_ag_address);
...
static void voice_confirm(GIOChannel *chan, gpointer data)
{
	struct audio_device *device;
	bdaddr_t src, dst;
	int perr;
	...
	if (!manager_allow_voice_connection(device)) {
		DBG("Refusing voice: too many existing connections");
		goto drop;
	}

	if (!device->voice) {
		btd_device_add_uuid(device->btd_dev, VOICE_SOURCE_UUID);
		if (!device->voice)
			goto drop;
	}

	if (voice_connect_rfcomm(device, chan) < 0) {
		error("voice_connect_rfcomm failed");
		goto drop;
	}

	voice_set_state(device, VOICE_STATE_CONNECTING);

	perr = audio_device_request_authorization(device, VOICE_UPLOAD_UUID,
						  voice_auth_cb, device);
	if (perr < 0) {
		DBG("Authorization denied: %s", strerror(-perr));
		voice_set_state(device, VOICE_STATE_DISCONNECTED);
		return;
	...
	device->voice_preauth_id = g_io_add_watch(chan,
					G_IO_NVAL | G_IO_HUP | G_IO_ERR,
					voice_preauth_cb, device);
...
static int voice_server_init(struct audio_adapter *adapter)
{
	uint8_t chan = DEFAULT_VOICE_UPLOAD_CHANNEL;
	sdp_record_t *record;
	gboolean master = TRUE;
	GError *err = NULL;
	GIOChannel *io;
	...
	io =  bt_io_listen(BT_IO_RFCOMM, NULL, voice_confirm, adapter, NULL, &err,
				BT_IO_OPT_SOURCE_BDADDR, &src,
				BT_IO_OPT_CHANNEL, chan,
				BT_IO_OPT_SEC_LEVEL, BT_IO_SEC_MEDIUM,
				BT_IO_OPT_MASTER, master,
	...
	adapter->voice_up_server = io;

	record = voice_up_record(chan);
	if (!record) {
		error("Unable to allocate new service record");
		goto failed;
	}
	...
	if (add_record_to_server(&src, record) < 0) {
		error("Unable to register Voice Upload service record");
		sdp_record_free(record);
		goto failed;
	}
	adapter->voice_up_record_id = record->handle;

	voice_config_init(config);
...
failed:
	...
	if (adapter->voice_up_server) {
		g_io_channel_shutdown(adapter->voice_up_server, TRUE, NULL);
		g_io_channel_unref(adapter->voice_up_server);
		adapter->voice_up_server = NULL;
	}
...
static int audio_probe(struct btd_device *device, GSList *uuids)
{
	...
	//if the remove device supports both source and sink,
	//cannot set both SRC and SNK at same time or AVTP
	//will fail.  Prioritize SNK over SRC for remote device
	//by removing SRC.
	GSList *l;
	gboolean isSink = FALSE;
	gboolean isSource = FALSE;
	//traverse list to see a2dp support
	for (l = uuids; l; l = l->next) {
		const char *uuidstr = l->data;
		if (strncmp(uuidstr, A2DP_SOURCE_UUID, MAX_LEN_UUID_STR) == 0) {
			isSource = TRUE;
		} else if (strncmp(uuidstr, A2DP_SINK_UUID, MAX_LEN_UUID_STR) == 0) {
			isSink = TRUE;
		}
	}

	//if both SRC and SNK exist, remove the SRC.
	if (isSink && isSource) {
		GSList *uuids_filtered = NULL;
		for (l = uuids; l; l = l->next) {
			const char *uuidstr = l->data;
			if (strncmp(uuidstr, A2DP_SOURCE_UUID, MAX_LEN_UUID_STR) != 0) {
		 		DBG("audio_probe, filter keep %s", uuidstr);
		        	uuids_filtered = g_slist_append(uuids_filtered, uuidstr);
			} else {
				DBG("audio_probe, filter out %s", uuidstr);
			}
		}
		g_slist_foreach(uuids_filtered, (GFunc) handle_uuid, audio_dev);
	} else {
...
static int voice_server_probe(struct btd_adapter *adapter)
{
	struct audio_adapter *adp;
	const gchar *path = adapter_get_path(adapter);
	int err;
...
static void voice_server_remove(struct btd_adapter *adapter)
{
	struct audio_adapter *adp;
	const gchar *path = adapter_get_path(adapter);
	...
	if (adp->voice_up_record_id) {
		remove_record_from_server(adp->voice_up_record_id);
		adp->voice_up_record_id = 0;
	}

	if (adp->voice_up_server) {
		g_io_channel_shutdown(adp->voice_up_server, TRUE, NULL);
		g_io_channel_unref(adp->voice_up_server);
		adp->voice_up_server = NULL;
	}
...
static struct btd_device_driver audio_driver = {
	...
	.uuids	= BTD_UUIDS(HSP_HS_UUID, HFP_HS_UUID, HSP_AG_UUID, HFP_AG_UUID,
			...
			AVRCP_TARGET_UUID, AVRCP_REMOTE_UUID, VOICE_UPLOAD_UUID,
			VOICE_SOURCE_UUID),
	.probe	= audio_probe,
	.remove	= audio_remove,
};
...
static struct btd_adapter_driver voice_server_driver = {
	.name	= "audio-voice",
	.probe	= voice_server_probe,
	.remove = voice_server_remove,
};
							...
							gboolean *enable_sco)
{
	...
	for (i = 0; list && list[i] != NULL; i++) {
		if (g_str_equal(list[i], "Headset"))
			...
			enabled.media = TRUE;
		else if (g_str_equal(list[i], "Voice"))
			enabled.voice = TRUE;
...
proceed:
	...
	if (enabled.voice)
		btd_register_adapter_driver(&voice_server_driver);

	btd_register_device_driver(&audio_driver);

	*enable_sco = (enabled.gateway || enabled.headset || enabled.voice);
					...
					gboolean connected)
{
	...
	for (l = devices; l != NULL; l = l->next) {
		...
		if (interface && !strcmp(AUDIO_VOICE_INTERFACE, interface)
				&& !dev->voice)
			continue;
...
gboolean manager_allow_voice_connection(struct audio_device *device)
{
	return TRUE;
}
