...
{
	struct serial_device *device = data;
	char ba[18], obf_ba[18];

	ba2str(&device->dst, ba);
	str2obfstr(ba, obf_ba);
	DBG("Unregistered interface %s for device %s", SERIAL_PORT_INTERFACE,
		obf_ba);
