...
static void mgmt_device_connected(int sk, uint16_t index, void *buf, size_t len)
{
	...
	DBG("hci%u device %s connected eir_len %u", index, obf_addr, eir_len);
								...
								size_t len)
{
	...
	DBG("hci%u device %s disconnected", index, obf_addr);
...
static void mgmt_connect_failed(int sk, uint16_t index, void *buf, size_t len)
{
	...
	DBG("hci%u %s status %u", index, obf_addr, ev->status);
								...
								size_t pin_len)
{
	...
	ba2str(bdaddr, addr);
	str2obfstr(addr, obf_addr);
	DBG("index %d addr %s pinlen %zu", index, obf_addr, pin_len);
							...
							gboolean success)
{
	...
	ba2str(bdaddr, addr);
	str2obfstr(addr, obf_addr);
	DBG("index %d addr %s success %d", index, obf_addr, success);
							...
							uint32_t passkey)
{
	...
	ba2str(bdaddr, addr);
	str2obfstr(addr, obf_addr);
	DBG("index %d addr %s passkey %06u", index, obf_addr, passkey);
...
static int mgmtops_read_voice_setting(int index, uint16_t *vs)
{
	return -ENOSYS;
}

static int mgmtops_write_voice_setting(int index, uint16_t vs)
{
	return -ENOSYS;
}
...
static struct btd_adapter_ops mgmt_ops = {
	...
	.read_voice_setting = mgmtops_read_voice_setting,
	.write_voice_setting = mgmtops_write_voice_setting,
};
