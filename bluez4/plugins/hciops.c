...
#define VOICE_SETTING_MASK 0x3FFU
...
static struct dev_info {
	...
	uint16_t voice_setting_pending;
} *devs = NULL;
...
static int resolve_name(struct dev_info *info, bdaddr_t *bdaddr)
{
	...
	DBG("hci%d dba %s", info->id, obf_addr);
...
static int get_handle(int index, bdaddr_t *bdaddr, uint16_t *handle)
{
	...
	ba2str(bdaddr, addr);
	str2obfstr(addr, obf_addr);
	DBG("hci%d dba %s", index, obf_addr);
...
static void link_key_request(int index, bdaddr_t *dba)
{
	...
	struct bt_conn *conn;
	GSList *match;
	char da[18], obf_addr[18];

	ba2str(dba, da);
	str2obfstr(da, obf_addr);
	DBG("hci%d dba %s", index, obf_addr);
...
static void link_key_notify(int index, void *ptr)
{
	...
	struct bt_conn *conn;
	GSList *match;
	char da[18], obf_da[18];
	uint8_t status = 0;

	ba2str(dba, da);
	str2obfstr(da, obf_da);
	DBG("hci%d dba %s type %d", index, obf_da, evt->key_type);
...
static void return_link_keys(int index, void *ptr)
{
	...
	for (i = 0; i < num; i++) {
		...
		str2obfstr(da, obf_da);
		DBG("hci%d returned key for %s", index, obf_da);
							...
							gboolean success)
{
	...
	ba2str(bdaddr, addr);
	str2obfstr(addr, obf_addr);
	DBG("hci%d dba %s success %d", index, obf_addr, success);
...
static void user_confirm_request(int index, void *ptr)
{
	...
	/* This section bypasses the User Agent for NoInputNoOutput
	 * devices. However, for our application we would like all
	 * pairing requests to go through the User Agent, even if MITM
	 * is not required.
	 */
#ifdef BYPASS_AGENT_FOR_NINO_PAIRING
...
#define ALWAYS_USE_BONDING
#ifdef ALWAYS_USE_BONDING
		/* If remote indicates no bonding then ask for bonding anyway.
		...
		/**
		* Local Capability: No Input No Output;        Pairing: Just Works
		* Local Capability: Some Input/Some Output;    Pairing: MITM Protection
		* If local capability is not NoInputNoOutput, then ask for
		* MITM protection
		*/
		if (conn->loc_cap != 0x03) {
			conn->loc_auth |= 0x01;
		}
	}
...
static void io_capa_request(int index, void *ptr)
{
	...
	ba2str(dba, da);
	str2obfstr(da, obf_addr);
	DBG("hci%d IO capability request for %s", index, obf_addr);
...
static void io_capa_response(int index, void *ptr)
{
	...
	str2obfstr(da, obf_addr);
	DBG("hci%d IO capability response from %s", index, obf_addr);
...
static void pin_code_request(int index, bdaddr_t *dba)
{
	...
	ba2str(dba, addr);
	str2obfstr(addr, obf_addr);
	DBG("hci%d PIN request for %s", index, obf_addr);
...
static void read_voice_setting_complete(int index, uint8_t status, read_voice_setting_rp *rp)
{
	struct dev_info *dev = &devs[index];

	dev->voice_setting = btohs(rp->voice_setting);
}

static void write_voice_setting_complete(int index, uint8_t status)
{
	struct dev_info *dev = &devs[index];

	if (status) {
		error ("Failed to write voice setting with status 0x%02x", status);
		return;
	}

	dev->voice_setting = dev->voice_setting_pending;
	dev->voice_setting_pending = VOICE_SETTING_INVALID;
}
...
static inline void cmd_complete(int index, void *ptr)
{
	...
	case cmd_opcode_pack(OGF_HOST_CTL, OCF_READ_VOICE_SETTING):
		ptr += sizeof(evt_cmd_complete);
		read_voice_setting_complete(index, status, ptr);
		break;
	case cmd_opcode_pack(OGF_HOST_CTL, OCF_WRITE_VOICE_SETTING):
		write_voice_setting_complete(index, status);
		break;
	};
}
						...
						uint16_t *accuracy)
{
	...
	ba2str(bdaddr, addr);
	str2obfstr(addr, obf_addr);
	DBG("hci%d addr %s which %d timeout %d", index, obf_addr, which, timeout);
					...
					uint8_t *hash, uint8_t *randomizer)
{
	...
	ba2str(bdaddr, addr);
	str2obfstr(addr, obf_addr);
	DBG("hci%d bdaddr %s", index, obf_addr);
				...
				uint8_t bdaddr_type, gboolean name_known)
{
	...
	ba2str(bdaddr, addr);
	str2obfstr(addr, obf_addr);
	DBG("hci%u %s name_known %u", index, obf_addr, name_known);
...
static int hciops_read_voice_setting(int index, uint16_t *vs)
{
	struct dev_info *dev = &devs[index];

	if (dev->voice_setting == VOICE_SETTING_INVALID) {
		if (hci_send_cmd(dev->sk, OGF_HOST_CTL, OCF_READ_VOICE_SETTING,
				 0, 0) < 0) {
			return -errno;
		}
		return -EAGAIN;
...
static int hciops_write_voice_setting(int index, uint16_t vs)
{
	struct dev_info *dev = &devs[index];
	write_voice_setting_cp cp;

	cp.voice_setting = htobs(vs);
	dev->voice_setting_pending = vs & VOICE_SETTING_MASK;

	if (hci_send_cmd(dev->sk, OGF_HOST_CTL, OCF_WRITE_VOICE_SETTING,
			 WRITE_VOICE_SETTING_CP_SIZE, &cp) < 0) {
		return -errno;
	}
...
static struct btd_adapter_ops hci_ops = {
	...
	.read_voice_setting = hciops_read_voice_setting,
	.write_voice_setting = hciops_write_voice_setting,
};
