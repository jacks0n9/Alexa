                   ...
                   void *data, int size)
{
    ...
    *(int *)data = get_nl_cmd(*(int *)data);
    if (*(int *)data == NL80211_WMI_BT_MAX) {
        A_INFO("WMI command not support\n");
        return A_ERROR;
    }
    ...
    if ((ret = ioctl(pAbfWlanInfo->Handle, ATH6KL_IOCTL_STANDARD03, &ifr)) < 0) {
...
void Abf_WlanCheckSettings(A_CHAR *wifname)
{
    ...
    if (wifname[0] == '\0' && property_get("wifi.interface", ifprop, NULL)) {
        strncpy(wifname, ifprop, IFNAMSIZ);
    }
#else
    {
        A_BOOL found = FALSE;
        ...
        if (f) {
            while(fgets(linebuf, sizeof(linebuf)-1, f)) {
                if (strchr(linebuf, ':')) {
                    ...
                    *dest = '\0';
                    if (firstif[0] == '\0') {
                        strncpy(firstif, ifname, IFNAMSIZ);
                    }
            ...
            if (!found && firstif[0] != '\0') {
                strncpy(wifname, firstif, IFNAMSIZ);
            }
            fclose(f);
        }
    }
