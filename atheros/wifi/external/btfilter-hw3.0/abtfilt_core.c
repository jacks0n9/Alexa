...
#define A2DP_CONFIG_A2DP_IS_HIGH_PRI          ( 1 << 3)
 ...
 //               pScoGenericConfig->scoFlags |= WMI_SCO_CONFIG_FLAG_ALLOW_OPTIMIZATION;
        ...
        if (pActionMsg->IndicationForControlAction == ATH_BT_A2DP) {
            ...
            //force a2dpMinBurstCnt to 7
            pA2dpPspollConfig->a2dpMinBurstCnt = 7;
...
extern void AdjustPageScanAction(void* pAbfBtInfo, WMI_SET_BTCOEX_BT_OPERATING_STATUS_CMD* pOperatingStatus);
        if (pActionMsg->IndicationForControlAction == ATH_BT_PAGESCAN) {
            WMI_SET_BTCOEX_BT_OPERATING_STATUS_CMD   *pOperatingStatus =
                (WMI_SET_BTCOEX_BT_OPERATING_STATUS_CMD *)(pActionMsg->ControlAction.Buffer);

            AdjustPageScanAction(pInfo->pBtInfo, pOperatingStatus);
        }
    }
    else
    {
        ...
        if (pActionMsg->IndicationForControlAction == ATH_BT_A2DP) {
			...
			A_INFO("DownGrading lmp version\n");
                }
                               ...
                               ATHBT_STATE State, unsigned char LMPVersion)
{
    ...
    if ((Indication == ATH_BT_NOOP) || (Indication == ATH_BT_PAGESCAN)) {
        return;
    }
...
HandleAdapterEvent(ATHBT_FILTER_INFO *pInfo, ATH_ADAPTER_EVENT Event)
{
    ...
    switch (Event) {
        case ATH_ADAPTER_ARRIVED:
            ...
            Abf_WlanCheckSettings(pInfo->pWlanInfo->IfName);
...
ExecuteBtAction(ATHBT_FILTER_INFO *pInfo, BT_ACTION_MSG *pBtActionMsg)
{
    ...
    if(pInfo->Flags & (ABF_WIFI_CHIP_IS_VENUS | ABF_WIFI_CHIP_IS_MCKINLEY)) {
        ...
        /* HID coex is the newly-added command. Use new path to issue the WMI command */
