...
A_UINT8  g_connected_HID_num = 0;
A_UINT8  g_sniff_HID_num = 0;
A_UINT16 g_HIDConnHandle = 0;
A_UINT16 g_HIDDisconnHandle = 0;
#define HID_ACT_THRESHOLD  0x50
#define HID_SUPPORT_NUM        6
struct HID_record{
    A_UINT16 handle;
    A_UINT8  sniff;
};
struct HID_record hid_conn[HID_SUPPORT_NUM] = {0};
...
typedef struct  _BT_CB_SV_PARAMS {
    const char* arg;
    int type;
    const void* val;
} BT_CB_SV_PARAMS;
...
static BT_NOTIFICATION_CONFIG_PARAMS g_NotificationConfig[BT_EVENTS_NUM_MAX] =
{
    ...
    {NULL, INVALID_INTERFACE, ARG_INVALID},
    /* INPUT_DEVICE_PROPERTYCHANGED */
    {"PropertyChanged", INPUT_DEVICE_INTERFACE, ARG_SV},
    /* AUDIO_SOURCE_PROPERTYCHANGED */
    {"PropertyChanged", AUDIO_SOURCE_INTERFACE, ARG_SV},
    /* DEVICE_PROPERTYCHANGED */
    {"PropertyChanged", ADAPTER_INTERFACE, ARG_SV},
};
...
static void DevicePropertyChanged(void *arg, void * user_data);
...
#endif
static void AudioSourcePropertyChanged(void* arg, void * user_data);
...
static A_STATUS CheckPageScan(ABF_BT_INFO *pAbfBtInfo);
...
void ConnHIDDevice(A_UINT16 handle){
	...
	for (i=0; i<HID_SUPPORT_NUM; i++){
		if (hid_conn[i].handle == 0) {
			...
			A_INFO("add HID conn idx:%d  handle:%x\n", i, handle);
			break;
		}
	}
...
void DisconnHIDDevice(A_UINT16 handle){
	...
	for (i=0; i<HID_SUPPORT_NUM; i++){
		if (hid_conn[i].handle == handle) {
			hid_conn[i].handle = 0;
			A_INFO("del HID conn idx:%d  handle:%x\n", i, handle);
			return;
		}
	}
}

/* mode: 0:active, 1:hold, 2:sniff, 3:park*/
void HIDModeChangeRecord(A_UINT16 handle, A_UINT8 mode) {
	A_UINT8 i = 0;

	for (i=0; i<HID_SUPPORT_NUM; i++){
		if (hid_conn[i].handle == handle) {
			hid_conn[i].sniff = mode;
			return;
		}
	}
}

A_BOOL HIDDeviceActive(void){
	A_UINT8 i = 0;

	for (i=0; i<HID_SUPPORT_NUM; i++){
		if ((hid_conn[i].handle!=0) && (hid_conn[i].sniff==0))
			return TRUE;
	}
	return FALSE;
}
...
static void
InputDevicePropertyChanged(void* arg, void * user_data)
{
    ABF_BT_INFO *pAbfBtInfo = (ABF_BT_INFO *)user_data;
    ATHBT_FILTER_INFO *pInfo = (ATHBT_FILTER_INFO *)pAbfBtInfo->pInfo;
    ATH_BT_FILTER_INSTANCE *pInstance = pInfo->pInstance;
    BT_CB_SV_PARAMS*  params = (BT_CB_SV_PARAMS*)arg;
	A_STATUS status;

    if (strncmp(params->arg, "Connected", sizeof("Connected")) != 0) {
        return;
    }

    if ((A_BOOL)(params->val)) {
        A_DEBUG("HID Connected\n");
		ConnHIDDevice(g_HIDConnHandle);
        AthBtIndicateState(pInstance, ATH_BT_HID, STATE_ON);
        AthBtIndicateState(pInstance, ATH_BT_PAGESCAN, STATE_OFF);
    }
    else {
        A_DEBUG("HID Disconnected\n");
        //AthBtIndicateState(pInstance, ATH_BT_HID, STATE_OFF);

        status = CheckPageScan(pAbfBtInfo);
        if (A_SUCCESS(status)) {
            AthBtIndicateState(pInstance, ATH_BT_PAGESCAN, STATE_OFF);
            if ((pAbfBtInfo->PageScanStatus & 0x2) != 0) {
                AthBtIndicateState(pInstance, ATH_BT_PAGESCAN, STATE_ON);
            }
        }
    }
}
...
static void
DevicePropertyChanged(void* arg, void * user_data)
{
    ABF_BT_INFO *pAbfBtInfo = (ABF_BT_INFO *)user_data;
    ATHBT_FILTER_INFO *pInfo = (ATHBT_FILTER_INFO *)pAbfBtInfo->pInfo;
    ATH_BT_FILTER_INSTANCE *pInstance = pInfo->pInstance;
    ...
    if (strncmp(params->arg, "Pairable", sizeof("Pairable")) != 0) {
        return;
    }

    CheckPageScan(pAbfBtInfo);
    if ((A_BOOL)(params->val)) {
        A_DEBUG("Pairable enabled\n");
        AthBtIndicateState(pInstance, ATH_BT_PAGESCAN, STATE_ON);
    }
    else {
        A_DEBUG("Pairable disabled\n");
        AthBtIndicateState(pInstance, ATH_BT_PAGESCAN, STATE_OFF);
    }
}
...
#endif
        DeRegisterBtStackEventCb(pAbfBtInfo, AUDIO_SOURCE_PROPERTYCHANGED);
...
#endif /* !BLUEZ4_3 */
        RegisterBtStackEventCb(pAbfBtInfo, AUDIO_SOURCE_PROPERTYCHANGED, (BT_EVENT_HANDLER)AudioSourcePropertyChanged);
...
static void
AudioSourcePropertyChanged(void *arg, void * user_data)
{
    ABF_BT_INFO *pAbfBtInfo = (ABF_BT_INFO *)user_data;
    ATHBT_FILTER_INFO *pInfo = (ATHBT_FILTER_INFO *)pAbfBtInfo->pInfo;
    ATH_BT_FILTER_INSTANCE *pInstance = pInfo->pInstance;
    ...
    if (strncmp(params->arg, "State", sizeof("State")) != 0) {
        return;
    }

    if (params->type != DBUS_TYPE_STRING) {
        return;
    }

    A_DEBUG("Audio Source (%s) State changed %s\n",pAbfBtInfo->DefaultRemoteAudioDeviceAddress, (char*)params->val);

    if (strncmp((char*)params->val, "connected", sizeof("connected")) == 0) {
        if (!(pInfo->Flags & ABF_USE_ONLY_DBUS_FILTERING)) {
            GetBtAudioDeviceProperties(pAbfBtInfo);
        }
        ...
        pInfo->Flags |= ABF_A2DP_SOURCE;
        AthBtIndicateState(pInstance, ATH_BT_A2DP, STATE_OFF);
    } else if (strncmp((char*)params->val, "playing", sizeof("playing")) == 0) {
        if (!pAbfBtInfo->DefaultRemoteAudioDevicePropsValid) {
            GetBtAudioDeviceProperties(pAbfBtInfo);
        }
        GetBtAudioConnectionProperties(pAbfBtInfo, ATH_BT_A2DP);
        AthBtIndicateState(pInstance, ATH_BT_A2DP, STATE_ON);
    } else if (strncmp((char*)params->val, "disconnected", sizeof("disconnected")) == 0) {
        AthBtIndicateState(pInstance, ATH_BT_A2DP, STATE_OFF);
        ForgetRemoteAudioDevice(pAbfBtInfo);
        pInfo->Flags &= ~ABF_A2DP_SOURCE;
    }
...
static A_STATUS ProcessDBusMessage(ABF_BT_INFO *pAbfBtInfo, DBusMessage *Msg)
{
    ...
    do {
        ...
        if (pNotificationConfig->arg == ARG_NONE) {
        ...
        } else if (pNotificationConfig->arg == ARG_SV) {
            DBusMessageIter subiter;
            BT_CB_SV_PARAMS params;
            int valType;
            void* val;

            if (!dbus_message_iter_init(Msg, &msgiter)) {
                A_ERR("[%s] event: %d (if:%s , sig: %s) no arguments \n",
                        __FUNCTION__, i, pNotificationConfig->interface,
                        pNotificationConfig->signal_name );
                break;
            }
            ...
            dbus_message_iter_get_basic(&msgiter, &argString);
            if ((i == DEVICE_PROPERTYCHANGED) && ((argType != DBUS_TYPE_STRING) || strcmp(argString, "Pairable")))
                break;
            dbus_message_iter_next(&msgiter);

            dbus_message_iter_recurse(&msgiter, &subiter);
            valType = dbus_message_iter_get_arg_type(&subiter);
            dbus_message_iter_get_basic(&subiter, &val);
            params.arg = argString;
            params.type = valType;
            params.val = val;
            pAbfBtInfo->SignalHandlers[i](&params, pAbfBtInfo);
        }
...
AcquireBtAdapter(ABF_BT_INFO *pAbfBtInfo)
{
    ...
    do {
        ...
        RegisterBtStackEventCb(pAbfBtInfo, INPUT_DEVICE_PROPERTYCHANGED, (BT_EVENT_HANDLER)InputDevicePropertyChanged);
        RegisterBtStackEventCb(pAbfBtInfo, DEVICE_PROPERTYCHANGED, (BT_EVENT_HANDLER)DevicePropertyChanged);
...
ReleaseBTAdapter(ABF_BT_INFO *pAbfBtInfo)
{
    ...
    if (pAbfBtInfo->AdapterCbRegistered) {
        ...
        DeRegisterBtStackEventCb(pAbfBtInfo, INPUT_DEVICE_PROPERTYCHANGED);
        DeRegisterBtStackEventCb(pAbfBtInfo, DEVICE_PROPERTYCHANGED);
    }
...
static A_STATUS CheckPageScan(ABF_BT_INFO *pAbfBtInfo)
{
    ...
    A_UCHAR *eventPtr;
    int eventLen;
    read_scan_enable_rp* scan_enable_p;
    read_page_activity_rp* page_scan_act_p;
    read_inquiry_mode_rp* page_scan_type_p;

    pAbfBtInfo->PageScanStatus = 0x0;

    do {
        status = IssueHCICommand(pAbfBtInfo,
                        cmd_opcode_pack(OGF_HOST_CTL, OCF_READ_SCAN_ENABLE),
                        NULL,
                        0,
        ...
        if (A_FAILED(status)) {
            A_ERR("[%s] Failed to read scan enable\n", __FUNCTION__);
            break;
        }

        scan_enable_p = (read_scan_enable_rp*)&evtBuffer[6];

        if (scan_enable_p->status != 0x0) {
            A_ERR("[%s] Failed to read scan eanble status:0x%x\n", __FUNCTION__, scan_enable_p->status);
            status = A_ERROR;
            break;
        }

        pAbfBtInfo->PageScanStatus = scan_enable_p->enable;
        A_INFO("[%s] Page Scan:%u \n", __FUNCTION__, pAbfBtInfo->PageScanStatus);


        status = IssueHCICommand(pAbfBtInfo,
                        cmd_opcode_pack(OGF_HOST_CTL, OCF_READ_PAGE_SCAN_TYPE),
                        NULL,
                        0,
                        HCI_REMOTE_COMMAND_TIMEOUT,
                        evtBuffer,
        ...
        if (A_FAILED(status)) {
            A_ERR("[%s] Failed to read scan type\n", __FUNCTION__);
            break;
        }

        page_scan_type_p = (read_inquiry_mode_rp*)&evtBuffer[6];

        if (page_scan_type_p->status != 0x0) {
            A_ERR("[%s] Failed to read scan type status:0x%x\n", __FUNCTION__, page_scan_type_p->status);
            status = A_ERROR;
            break;
        }

        pAbfBtInfo->PageScanType = page_scan_type_p->mode;
        A_INFO("[%s] Page Scan type:0x%x \n", __FUNCTION__, pAbfBtInfo->PageScanType);

        status = IssueHCICommand(pAbfBtInfo,
                        cmd_opcode_pack(OGF_HOST_CTL, OCF_READ_PAGE_ACTIVITY),
                        (A_UCHAR *)&conn_handle,
                        2,
                        HCI_REMOTE_COMMAND_TIMEOUT,
                        evtBuffer,
        ...
        if (A_FAILED(status)) {
            A_ERR("[%s] Failed to read page scan activity\n", __FUNCTION__);
            break;
        }

        page_scan_act_p = (read_page_activity_rp*)&evtBuffer[6];

        if (page_scan_act_p->status != 0x0) {
            A_ERR("[%s] Failed to read page scan activity:0x%x\n", __FUNCTION__, page_scan_act_p->status);
            status = A_ERROR;
            break;
        }
        

        pAbfBtInfo->PageScanInterval = page_scan_act_p->interval;
        pAbfBtInfo->PageScanWindow = page_scan_act_p->window;
        A_INFO("[%s] Page Scan: Interval:%u slots Window:%u slots\n", __FUNCTION__, pAbfBtInfo->PageScanInterval, pAbfBtInfo->PageScanWindow);
...
void AdjustPageScanAction(void* info, WMI_SET_BTCOEX_BT_OPERATING_STATUS_CMD* pOperatingStatus)
{
    ABF_BT_INFO *pAbfBtInfo = (ABF_BT_INFO*)info;
    pOperatingStatus->btLinkId = pAbfBtInfo->PageScanInterval << 16;
    if (pAbfBtInfo->PageScanType == 0x1) {
        pOperatingStatus->btLinkId |= pAbfBtInfo->PageScanWindow*2;
    } else {
        pOperatingStatus->btLinkId |= pAbfBtInfo->PageScanWindow;
    }
}
...
A_STATUS IssueHighCmd(ATHBT_FILTER_INFO * pInfo, A_BOOL val);

static void CleanupHciEventFilter(ABF_BT_INFO *pAbfBtInfo)
{
    A_STATUS status;
    struct itimerspec ct;
    ATHBT_FILTER_INFO *pInfo = (ATHBT_FILTER_INFO *)pAbfBtInfo->pInfo;

    if (timer_gettime(timerid, &ct) == 0) {
        timer_delete(timerid);
        IssueHighCmd(pInfo, FALSE);
    }
...
static A_STATUS SetupHciEventFilter(ABF_BT_INFO *pAbfBtInfo)
{
    ...
    do {
        ...
        set_event(EVT_DISCONN_COMPLETE, &filterSetting);
        set_event(EVT_MODE_CHANGE, &filterSetting);
        set_event(EVT_ROLE_CHANGE, &filterSetting);
...
static WLAN_CHANNEL_MAP g_ChannelMapTable[MAX_WLAN_CHANNELS + 1] = {
    ...
    { {0x00,0x00,0x00,0xE0,0xFF,0xFF,0xFF,0xFF,0xFF,0x7F}}, /* 1 */
    { {0x00,0x00,0x00,0x00,0xFC,0xFF,0xFF,0xFF,0xFF,0x7F}}, /* 2 */
    { {0x03,0x00,0x00,0x00,0x80,0xFF,0xFF,0xFF,0xFF,0x7F}}, /* 3 */
    { {0x7F,0x00,0x00,0x00,0x00,0xF0,0xFF,0xFF,0xFF,0x7F}}, /* 4 */
    { {0xFF,0x0F,0x00,0x00,0x00,0x00,0xFE,0xFF,0xFF,0x7F}}, /* 5 */
    { {0xFF,0xFF,0x01,0x00,0x00,0x00,0xC0,0xFF,0xFF,0x7F}}, /* 6 */
    { {0xFF,0xFF,0x3F,0x00,0x00,0x00,0x00,0xF8,0xFF,0x7F}}, /* 7 */
    { {0xFF,0xFF,0xFF,0x07,0x00,0x00,0x00,0x00,0xFF,0x7F}}, /* 8 */
    { {0xFF,0xFF,0xFF,0xFF,0x00,0x00,0x00,0x00,0xE0,0x7F}}, /* 9 */
    { {0xFF,0xFF,0xFF,0xFF,0x1F,0x00,0x00,0x00,0x00,0x7C}}, /* 10 */
    { {0xFF,0xFF,0xFF,0xFF,0xFF,0x03,0x00,0x00,0x00,0x00}}, /* 11 */
    { {0xFF,0xFF,0xFF,0xFF,0xFF,0x7F,0x00,0x00,0x00,0x00}}, /* 12 */
    { {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x0F,0x00,0x00,0x00}}, /* 13 */
    { {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x00,0x00}}, /* 14 */
};
...
#ifdef BLUEZ4_3
A_STATUS
IssueHighCmd(ATHBT_FILTER_INFO * pInfo, A_BOOL val)
{
    WMI_SET_BTCOEX_BT_OPERATING_STATUS_CMD opcmd;
    A_UINT32  buf[sizeof(A_UINT32) + sizeof(WMI_SET_BTCOEX_BT_OPERATING_STATUS_CMD)];
    A_STATUS status;

    /* Set co-located bt type to 1, generic for any PTA based bluetooth */
    buf[0] = AR6000_XIOCTL_WMI_SET_BT_OPERATING_STATUS;
    opcmd.btProfileType = 11;
    opcmd.btOperatingStatus = (val == TRUE)?1:2;
    opcmd.btLinkId = 0x0;

    A_MEMCPY(&buf[1], (void *)&opcmd, sizeof(opcmd));
    A_INFO("High Priority BT Cmd\n");
    status = Abf_WlanDispatchIO(pInfo, AR6000_IOCTL_EXTENDED,
                (void *)buf,
                (sizeof(buf)));

    if (A_FAILED(status)) {
        A_ERR("[%s] Failed to issue High Priority BT Cmd\n", __FUNCTION__);
        return A_ERROR;
    }
    return A_OK;
}
...
{
	ATHBT_FILTER_INFO *pInfo = (ATHBT_FILTER_INFO *)v.sival_ptr;
...
static void *HCIFilterThread(void *arg)
{
    ...
    sev.sigev_value.sival_ptr = pInfo;
    sev.sigev_notify = SIGEV_THREAD;
    sev.sigev_notify_function = timer_thread;
    if (timer_create(CLOCK_MONOTONIC, &sev, &timerid) == -1) {
        A_ERR("fail to create timer");
    }
    ...
    while (1) {
        ...
        if (pBuffer[0] == HCI_EVENT_PKT) {
            ...
            if(*eventCode == EVT_CONN_COMPLETE) {
                ...
                if(pBuffer[12] == 0x01) //ACL connection
                    g_HIDConnHandle = (pBuffer[4] | (pBuffer[5] << 8));
            }
            ...
            if (*eventCode == EVT_DISCONN_COMPLETE) {
                g_HIDDisconnHandle = (pBuffer[4] | (pBuffer[5] << 8));
                DisconnHIDDevice(g_HIDDisconnHandle);
                /* Since Mode Chage event(active) may be received during
                disconnection and enable HID coex, disable HID coex if there's
                no active HID device after removing the disconnected device.*/
                if(!HIDDeviceActive()){
                    A_INFO("all HID in sniff mode after disconnct!\n");
                    AthBtIndicateState(pInstance,
                        ATH_BT_HID, STATE_OFF);
                }
            }

            if (*eventCode == EVT_ROLE_CHANGE)
			{
				A_INFO("role change event received!");
				if (timer_gettime(timerid, &ct) == -1) {
					A_ERR("%d:timer gettime failed\n", __LINE__);
				} else {
					...
					if (timer_settime(timerid, 0, &it, NULL) == -1) {
						A_ERR("%d:timer settime failed\n", __LINE__);
					}
				}
				IssueHighCmd(pInfo, FALSE);
			}

            if (*eventCode == EVT_MODE_CHANGE)
            {
                A_UINT16 handle = (pBuffer[4] | (pBuffer[5] << 8));
                A_UINT16 interval = (pBuffer[7] | (pBuffer[8] << 8));

                /* status 0: a mode change has occured */

                if(pBuffer[3] == 0) {
                    if ((pBuffer[6] != 0) &&
                        (interval < HID_ACT_THRESHOLD)) {
                        HIDModeChangeRecord(handle, 0);
                    }
                    else
                        HIDModeChangeRecord(handle, pBuffer[6]);

                    if(!HIDDeviceActive()){
                        A_INFO("all HID in sniff mode!\n");
                        AthBtIndicateState(pInstance,
                        ATH_BT_HID, STATE_OFF);
                    }
                    else{
                        A_INFO("HID active mode!\n");
                        AthBtIndicateState(pInstance,
                        ATH_BT_HID, STATE_ON);
                    }
                }
   ...
   //             AthBtFilterHciEvent(pInstance,&pBuffer[1],eventLen - 1);
            ...
            if ((*packedOpCode == 0x405) || (*packedOpCode == 0x409)) {
                A_INFO("Role Started role:0x%x\n", pBuffer[10]);
                if (timer_gettime(timerid, &ct) == -1) {
                    A_ERR("%d:timer gettime failed\n", __LINE__);
                } else {
                    it.it_interval.tv_sec = 0;
                    ...
                    if (ct.it_value.tv_sec == 0 && ct.it_value.tv_nsec == 0) {
                        IssueHighCmd(pInfo, TRUE);
                    }
                }
