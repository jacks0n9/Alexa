        ...
        | LL_IDENTIFIER
          {
            ...
            node = (FilterExprNode *) plugin_parse_config(p, configuration, &@1);
            free($1);
            if (!node)
              {
                YYERROR;
	...
	| string operator string
          {
            ...
            left = log_template_new(configuration, NULL, $1);
            right = log_template_new(configuration, NULL, $3);
            free($1);
            free($3);
            CHECK_ERROR(log_template_compile(left, &error), @1, "error compiling template: %s", error->message);
            CHECK_ERROR(log_template_compile(right, &error), @3, "error compiling template: %s", error->message);
