...
/* Watchdog Read wait retry */
#define WD_WAIT_RETRY_COUNT		5
/* Watchdog Read wait delay */
#define WD_WAIT_DELAY		50
 ...
 *****************************************************************************/
void wait_for_command_complete(struct watchdog *wd_base, u32 wait_flag)
{
	int attempts = WD_WAIT_RETRY_COUNT;
	volatile unsigned int pending = 0;
	do {
		pending = readl(&wd_base->wwps) & wait_flag;
		if (pending) {
			delay(WD_WAIT_DELAY);
			attempts--;
		}
	} while (pending && attempts);
}
...
void watchdog_enable(void)
{
	...
	writel(WD_LOCK1, &wd2_base->wspr);
	wait_for_command_complete(wd2_base, WD_WWPS_WSPR_PENDING);
	writel(WD_LOCK2, &wd2_base->wspr);
	wait_for_command_complete(wd2_base, WD_WWPS_WSPR_PENDING);
}
...
void watchdog_set_timeout(u32 time_in_sec)
{
	...
	writel(timeout, &wd2_base->wldr);
	/* Ensure pending load is written */
	wait_for_command_complete(wd2_base, WD_WWPS_WLDR_PENDING);
	/* Trigger a reload */
	wdt_trgr_pattern = ~wdt_trgr_pattern;
	writel(wdt_trgr_pattern, &wd2_base->wtgr);
	wait_for_command_complete(wd2_base, WD_WWPS_WTGR_PENDING);
}

/******************************************************************************
 * Routine: watchdog_reset
 * Description: Reset the watchdog timer
 *****************************************************************************/
void watchdog_reset(void)
{
	watchdog_set_timeout(CONFIG_SYS_WATCHDOG_TIMEOUT);
	watchdog_enable();
}
...
void watchdog_init(void)
{
	...
	writel((1 << 5) | (WD_PTV << 2), &wd2_base->wclr);
	/* Ensure pending load is written */
