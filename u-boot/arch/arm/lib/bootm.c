...
#ifdef CONFIG_IDME_TAG
void setup_idme_tag(struct tag **params);
#endif
...
#ifdef CONFIG_REVISION16_TAG
extern const u8 *get_board_id(void);
...
void setup_macaddr_tag(struct tag **in_params)
{
	...
	if (idme_get_var("btmac", bt_buf, sizeof(bt_buf)))
	return;

	if (idme_get_var("mfg", sec_buf, sizeof(sec_buf)))
	return;

	params->hdr.tag = ATAG_MACADDR;
	params->hdr.size = tag_size (tag_macaddr);
	memcpy (params->u.macaddr.wifi_address, mac_buf, sizeof params->u.macaddr.wifi_address);
	memcpy (params->u.macaddr.bt_address, bt_buf, sizeof params->u.macaddr.bt_address);
...
void setup_idme_tag(struct tag **in_params)
{
#ifdef CONFIG_CMD_IDME
	/* Get the idme blob */
	char *buf;
	int blob_size = idme_get_blob(&buf);

	if (blob_size > 0 && blob_size <= sizeof(params->u.idme.data)) {
		/* Copy the data into the kernel tags */
		memcpy(params->u.idme.data, buf, blob_size);
		params->u.idme.size = blob_size;
		params->hdr.tag = ATAG_IDME;
		params->hdr.size = tag_size(tag_idme);
		params = tag_next(params);
	}
#endif /* CONFIG_CMD_IDME */
}
#endif  /* CONFIG_IDME_TAG */
