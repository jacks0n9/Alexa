...
/*******************************************************************/
static int do_bootm_impl (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	ulong		iflag;
	ulong		load_end = 0;
	int		ret;
...
/*******************************************************************/
/* bootm - Wrapper function */
/*******************************************************************/
int do_bootm (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	int ret = do_bootm_impl(cmdtp, flag, argc, argv);
#ifdef CONFIG_OMAP3_LAB126
	set_kernel_bootmode();
	/* Reset the device if bootm ever returns */
	do_reset (cmdtp, flag, argc, argv);
	/* If for whatever reason, reset fails, wait forever until we get watchdog slapped */
	printf ("Boot hang...\n");
	while(1) {}
#endif
	return ret;
