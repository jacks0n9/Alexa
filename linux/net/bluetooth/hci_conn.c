...
void hci_conn_enter_sniff_mode(struct hci_conn *conn)
{
	...
	link_policy = conn->link_policy_valid ? conn->link_policy : hdev->link_policy;

	if (conn->mode != HCI_CM_ACTIVE || !(link_policy & HCI_LP_SNIFF))
		return;
...
void set_page_scan_type(struct hci_dev *hdev, u8 pscan_type, u16 pscan_interval)
{
     ...
     * range which is between: 0x0012 to 0x1000 (11.25ms to 2.560secs)
     */
