...
static int l2cap_build_conf_req(struct sock *sk, void *data, size_t data_size);
...
static int l2cap_parse_conf_rsp(struct sock *sk, void *rsp, int len,
...
static inline int l2cap_config_req(struct l2cap_conn *conn, struct l2cap_cmd_hdr *cmd, u16 cmd_len, u8 *data)
{
	...
	/* Complete config. */
	len = l2cap_parse_conf_req(sk, rsp, sizeof(rsp));
	if (len < 0) {
...
static inline int l2cap_config_rsp(struct l2cap_conn *conn, struct l2cap_cmd_hdr *cmd, u8 *data)
{
	...
	case L2CAP_CONF_UNACCEPT:
		if (l2cap_pi(sk)->num_conf_rsp <= L2CAP_CONF_MAX_CONF_RSP) {
			...
			len = l2cap_parse_conf_rsp(sk, rsp->data, len,
