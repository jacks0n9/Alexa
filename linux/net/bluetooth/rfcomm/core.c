...
#define rfcomm_lock_if_user() if(current->mm){ BT_DBG("rfcomm_lock_if_user:locking\n"); rfcomm_lock(); }
#define rfcomm_unlock_if_user() if(current->mm){ BT_DBG("rfcomm_unlock_if_user:unlocking\n"); rfcomm_unlock(); }
...
static int rfcomm_session_is_valid(struct rfcomm_session *sess, struct rfcomm_dlc *d);
...
int rfcomm_dlc_close(struct rfcomm_dlc *d, int err)
{
	...
	if(rfcomm_session_is_valid(d->session, d))
...
static int rfcomm_session_is_valid(struct rfcomm_session *s, struct rfcomm_dlc *d)
{
	struct rfcomm_dlc *d_list;
	struct rfcomm_session *s_list;
...
void rfcomm_dlc_accept(struct rfcomm_dlc *d)
{
	...
	if(! rfcomm_session_is_valid(d->session, d)){
		rfcomm_unlock_if_user();
		return;
	}

	sk = d->session->sock->sk;
...
static int rfcomm_add_listener(bdaddr_t *ba)
{
	...
	BT_DBG("sock:%p sk:%p", sock, sock->sk);
	/* Bind socket */
