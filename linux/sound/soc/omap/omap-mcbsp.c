...
/* In "AIC3XXX_DSP_CONTROL" mode, the DSP is responsible for
 * configuring the McBSP, and not this driver.  However, other devices
 * using the McBSP /do/ need this driver.  Therefore the bitmask
 * omap_mcbsp_dai_config indicates which McBSP
 * ports/interfaces/instances should be configured by this driver.
 *
 * Note: this is a board-specific set-up.
 */
#ifndef CONFIG_AIC3XXX_DSP_CONTROL
static unsigned omap_mcbsp_dai_config = ~0;
#else
/* Bluetooth on McBSP5 (EVT2) or McBSP3 (EVT3+) */
static unsigned omap_mcbsp_dai_config = (1 << 4) | (1 << 2);
#endif

#define SHOULD_CONFIGURE_MCBSP(mcbsp_data) \
	(omap_mcbsp_dai_config & (1 << ((mcbsp_data)->bus_id)))
...
static void omap_mcbsp_set_threshold(struct snd_pcm_substream *substream)
{
	...
	if (!SHOULD_CONFIGURE_MCBSP(mcbsp_data))
		return;

	dma_data = snd_soc_dai_get_dma_data(rtd->cpu_dai, substream);

	dprintk (KERN_INFO "%s: RTD->CPU_DAI->NAME %s DMA_OP_MODE %d\n", __func__, cpu_dai->name, dma_op_mode);
	...
	dprintk (KERN_INFO "%s: Setting %d Threshold to %d\n", __func__, substream->stream, words);
				    ...
				    struct snd_soc_dai *cpu_dai)
{
	...
	default:
	   ...
	   regs->xcr1	|= XFRLEN1(wpf - 1);
           omap_mcbsp_set_xmt_frame_len(bus_id, &mcbsp_data->regs);
        }
        else
        {
	   regs->rcr1	|= RFRLEN1(wpf - 1);
           omap_mcbsp_set_rcv_frame_len(bus_id, &mcbsp_data->regs);
        }
	...
	mcbsp_data->configured = 1;
        ...
        dprintk (KERN_INFO "Mcbsp: %s Function Ending...\n", __func__);

	dprintk(KERN_INFO "mcbsp: dma = %d\tbus_id = %d\n", dma, bus_id);
	dprintk(KERN_INFO "mcbsp: port = 0x%x\n", port);
	dprintk(KERN_INFO "mcbsp: regs->srgr2 = 0x%x  regs->srgr1 = 0x%x\n", regs->srgr2, regs->srgr1);
	dprintk(KERN_INFO "mcbsp: regs->rcr2 = 0x%x  regs->rcr1 = 0x%x\n", regs->rcr2, regs->rcr1);
	dprintk(KERN_INFO "mcbsp: regs->xcr2 = 0x%x regs->xcr1 = 0x%x\n", regs->xcr2, regs->xcr1);
	dprintk(KERN_INFO "mcbsp: regs->spcr1 = 0x%x regs->spcr2 = 0x%x\n", regs->spcr1, regs->spcr2);

	dprintk("dma name:\t%s\n", dma_data->name);
	dprintk("dma req:\t%d\n", dma_data->dma_req);
	dprintk("dma port addr:\t0x%x\n", dma_data->port_addr);
	dprintk("dma type:\t%d\n", dma_data->data_type);
	dprintk("dma sync mode:\t%d\n", dma_data->sync_mode);
	dprintk("dma packet_size:\t%d\n",dma_data->packet_size);
	dprintk("----------------------------------------------------------\n");
				      ...
				      unsigned int fmt)
{
	...
	dprintk(KERN_INFO "mcbsp: regs->rcr2 = 0x%x  regs->rcr1 = 0x%x\n", regs->rcr2, regs->rcr1);
	dprintk(KERN_INFO "mcbsp: regs->xcr2 = 0x%x regs->xcr1 = 0x%x\n", regs->xcr2, regs->xcr1);
	dprintk(KERN_INFO "mcbsp: regs->pcr0 = 0x%x regs->spcr1 = 0x%x regs->spcr2 = 0x%x\n",
			regs->pcr0, regs->spcr1, regs->spcr2);

	dprintk (KERN_INFO "mcbsp: regs->mcr1 = 0x%x regs->mcr2 = 0x%x\n",
			regs->mcr1, regs->mcr2);
