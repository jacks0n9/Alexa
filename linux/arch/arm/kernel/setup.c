...
#if defined(CONFIG_MACH_OMAP3_MISTO)
...
unsigned char system_wifi_mac_addr[MAC_ADDR_SIZE];
EXPORT_SYMBOL(system_wifi_mac_addr);

unsigned char system_bt_mac_addr[MAC_ADDR_SIZE];
EXPORT_SYMBOL(system_bt_mac_addr);

unsigned char system_mac_sec[MAC_SEC_SIZE];
EXPORT_SYMBOL(system_mac_sec);

unsigned char system_idme[IDME_SIZE];
EXPORT_SYMBOL(system_idme);

unsigned int system_idme_size;
EXPORT_SYMBOL(system_idme_size);
#endif
...
{
	memcpy(system_wifi_mac_addr, tag->u.macaddr.wifi_address, MAC_ADDR_SIZE);
	memcpy(system_bt_mac_addr, tag->u.macaddr.bt_address, MAC_ADDR_SIZE);
...
{
	memcpy(system_idme, tag->u.idme.data, tag->u.idme.size);
	system_idme_size = tag->u.idme.size;
	return 0;
}
