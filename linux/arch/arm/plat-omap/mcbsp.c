...
void omap_mcbsp_set_xmt_frame_len(unsigned int id, const struct omap_mcbsp_reg_cfg *config)
{
        struct omap_mcbsp *mcbsp;
        ...
        dev_dbg(mcbsp->dev, "Setting XCR1 register of McBSP%d  phys_base: 0x%08lx\n",
                        mcbsp->id, mcbsp->phys_base);

        /* We write the given config */
        MCBSP_WRITE(mcbsp, XCR1, config->xcr1);
}
EXPORT_SYMBOL(omap_mcbsp_set_xmt_frame_len);

void omap_mcbsp_set_rcv_frame_len(unsigned int id, const struct omap_mcbsp_reg_cfg *config)
{
        struct omap_mcbsp *mcbsp;
        ...
        dev_dbg(mcbsp->dev, "Setting RCR1 register of McBSP%d  phys_base: 0x%08lx\n",
                        mcbsp->id, mcbsp->phys_base);

        /* We write the given config */
        MCBSP_WRITE(mcbsp, RCR1, config->rcr1);
}
EXPORT_SYMBOL(omap_mcbsp_set_rcv_frame_len);
