...
static ssize_t lp5521_help_program(struct file *file, char __user *userbuf,
				    size_t count, loff_t *ppos)
{
	char buf[250];
	unsigned int len;
	const char *help = "\n How to start/stop LP5521 program\n\n \
	(example) To start program,\n \
	echo 's' > /sys/kernel/debug/lp5521/program\n \
	To end the program,\n \
	echo 'e' > /sys/kernel/debug/lp5521/program\n";
...
static void lp5521_program_cmd(char cmd, struct lp5521_chip *chip)
{
	struct i2c_client *cl = chip->client;

	if (cmd == 's') {
		lp5521_write(cl, LP5521_REG_OP_MODE, 0x10); //load
		lp5521_write(cl, LP5521_REG_R_PROG_MEM, 0x03);
		lp5521_write(cl, LP5521_REG_R_PROG_MEM + 1, 0x7F);
		lp5521_write(cl, LP5521_REG_R_PROG_MEM + 2, 0x4D);
		lp5521_write(cl, LP5521_REG_R_PROG_MEM + 3, 0x00);
		lp5521_write(cl, LP5521_REG_R_PROG_MEM + 4, 0x03);
		lp5521_write(cl, LP5521_REG_R_PROG_MEM + 5, 0xFF);
		lp5521_write(cl, LP5521_REG_R_PROG_MEM + 6, 0x7F);
		lp5521_write(cl, LP5521_REG_R_PROG_MEM + 7, 0x00);
		/* wait low 4 secs more using branch cmd */
		lp5521_write(cl, LP5521_REG_R_PROG_MEM + 8, 0xA2);
		lp5521_write(cl, LP5521_REG_R_PROG_MEM + 9, 0x03);
		lp5521_write(cl, LP5521_REG_OP_MODE, 0x20); //run
		lp5521_write(cl, LP5521_REG_ENABLE, 0x60); //exec to run
	} else if (cmd == 'e') {
		lp5521_write(cl, LP5521_REG_ENABLE, 0x40); //exec to hold
	} else {
		printk(KERN_INFO "Invalid cmd : %c\n", cmd);
	}
}

static ssize_t lp5521_do_program(struct file *file,
				    const char __user *userbuf, size_t count,
				    loff_t *ppos)
{
	struct lp5521_chip *chip = file->private_data;
...
static const struct file_operations fops_program = {
	.open = lp5521_dbg_open,
	.read = lp5521_help_program,
	.write = lp5521_do_program,
	.owner = THIS_MODULE,
	.llseek = default_llseek,
};
...
static void lp5521_create_debugfs(struct lp5521_chip *chip)
{
	...
	dd->program = debugfs_create_file("program", S_IWUSR | S_IRUGO,
				      dd->dir, chip, &fops_program);
}
