 ...
 * @have_pdata: 1 if platform data struct is provided, 0 otherwise.
 * @pdata: A copy of the platform data struct.
 *
...
struct ks8851_net {
	...
	u8                      have_pdata;
	struct ks8851_platform_data pdata;
};
...
static void ks8851_init_mac(struct ks8851_net *ks)
{
	...
	if (ks->have_pdata && ks->pdata.have_mac) {
		memcpy(dev->dev_addr, ks->pdata.mac, ETH_ALEN);
	} else {
...
static int __devinit ks8851_probe(struct spi_device *spi)
{
	...
	if (spi->dev.platform_data) {
		memcpy(&ks->pdata, spi->dev.platform_data,
		       sizeof(struct ks8851_platform_data));
		ks->have_pdata = 1;
	}
	...
	netdev_info(ndev, "revision %d, MAC %s:%s:%s:%s:%x:%x, IRQ %d\n",
		    CIDER_REV_GET(ks8851_rdreg16(ks, KS_CIDER)),
		    "XX", "XX", "XX", "XX", ndev->dev_addr[4], ndev->dev_addr[5], ndev->irq);
