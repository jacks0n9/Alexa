...
usage(void)
{
    ...
    fprintf(stderr, "  -v   : WLAN chip is venus\n");
    fprintf(stderr, "  -3   : Front End is 2x2 WLAN and non-shared BT\n");
}
...
main(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "bsvandczxqt3f:w:")) != EOF) {
        switch (opt) {
        case 'n':
            daemonize = 0;
            break;
    ...
    do {
        ...
        if (nl80211_init(&g_AthBtFilterInstance.nlstate) != 0) {
	...
	nl80211_cleanup(g_AthBtFilterInstance.nlstate);
