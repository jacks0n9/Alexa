...
void art_printf(const char *fmt, ...)
{
    va_list ap;
...
void disable_tty(void)
{
    ...
    system("init q");
    /* Replace the inittab, in case of premature exit */
...
static int wmiSend(unsigned char *cmdBuf, unsigned int len, unsigned int totalLen, unsigned char version, bool resp)
{
    ...
    if ((err = tcmd_tx((char*)&tCmd, sizeof(tCmd), resp))) {
        art_printf("tcmd_tx had error: %s!\n", strerror(err));
        return 0;
    }
...
int sock_init(int port)
{
    ...
    if (sockid == -1) {
        perror(__FUNCTION__);
        art_printf("Create socket to PC failed\n");
        return -1;
    }
    ...
    if (listen(sockid, 4) == -1) {
        perror(__FUNCTION__);
        art_printf("Listen failed\n");
        close(sockid);
        return -1;
    }

    art_printf("Waiting for client to connect...\n");

    sinsize = sizeof(struct sockaddr_in);
    if ((cid = accept(sockid, (struct sockaddr *)&myaddr, &sinsize)) == -1) {
        art_printf("Accept failed\n");
        close(sockid);
        return -1;
    }
    ...
    if (res == -1) {
        art_printf("cannot set NOdelay for cid\n");
        close(sockid);
        return -1;
    }
    art_printf("Client connected!\n");
...
int sock_recv(int sockid, unsigned char *buf, int buflen)
{
    ...
    if (recvbytes == 0) {
        art_printf("Connection close!? zero bytes received\n");
        return -1;
    } else if (recvbytes > 0) {
        return recvbytes;
    }
...
static void art_print_buf(unsigned char *buf, unsigned int len)
{
    ...
    art_printf("Buffer len %d, buffer content: \n", len);
    for (i = 0; i < len; i++)
    {
        art_printf("%x ", buf[i]);
        if ((i % 15) == 15)
...
int com_init(char *comname)
{
    ...
    if (ttyFid == -1)
    {
        art_printf( "Error unable to open port: %s\n", comname );
        return -1;
    }
    ...
    // All done
    art_printf( "Done, port speed set on: %s\n", comname );
...
int com_send(int comfd, unsigned char *buf, int buflen)
{
    ...
    if (buflen > PLATFORM_UART_BUF_SIZE+3) {
        art_printf("send too many ACK back to host%d\n",buflen);
        return -1;
    }
...
int com_recv(int comfd, unsigned char *buf,  unsigned char *type)
{
    ...
    *type = tempBuf[1];
    ...
    if (crc != crc_cal)
    {
        art_printf("ERRO! CRC error %x (cal_crc:%x), len =%d\n",crc, crc_cal, len);
        art_print_buf(buf, len);
        return -1;
    }
...
static void print_help(char *pname)
{
    ...
    art_printf("An agent program to connect ART host and AR6K device, must be\n");
    art_printf("started after AR6K device driver is installed.\n\n");
    art_printf("Usage: %s -d ifname -s fragsize -f log_file\n\n", pname);
    art_printf("  ifname      AR6K interface name\n");
    art_printf("  fragsize    Fragment size, must be multiple of 4\n\n");
    art_printf("  log_file    File to log output\n\n");
    art_printf("Example:\n");
    art_printf("%s -d eth1 -s 80 -f /tmp/log.txt\n\n", pname);
#endif
    /*
#ifdef  ART_PORT_COM
    art_printf("An agent that connects ART host and AR6K device. Must be started\n");
    art_printf("after AR6K device driver is successfully installed.\n\n");
    art_printf("Usage: %s ifname comx fragsize\n\n", pname);
    art_printf("  ifname    AR6K interface name\n");
    art_printf("  comx      COM port name\n");
    art_printf("  fragsize  Fragment size, must be multiple of 4\n\n");
    art_printf("Example:\n");
    art_printf("  %s eth0 /dev/ttyHSUSB0 80\n\n", pname);
#endif
 */
    ...
    art_printf("An agent that connects ART host and AR6K device. Must be started\n");
    art_printf("after AR6K device driver is successfully installed.\n\n");
    art_printf("Usage: %s -d comx -s fragsize -f log_file\n\n", pname);
    // art_printf("  ifname    AR6K interface name\n");
    art_printf("  comx      COM port name\n");
    art_printf("  fragsize  Fragment size, must be multiple of 4\n\n");
    art_printf("  log_file    File to log output\n\n");
    art_printf("Example:\n");
    art_printf("  %s -d /dev/ttyHSUSB0 -s 80 -f /tmp/logfile.log\n\n", pname);
#endif
}

static const char optionList[] = "d:s:f:";
...
int main (int argc, char **argv)
{
    ...
    while ((opt = getopt(argc, argv, optionList)) != -1)
    {
        ...
        {
        case 'd':
            // File name for shared memory buffer
            strncpy(comname, optarg, sizeof(comname));
            printf("comname = %s\n", comname);
            break;
        case 's':
    ...
    art_printf("setup signal\n");
    memset(&sa, 0, sizeof(struct sigaction));

    sa.sa_flags = SA_NOCLDSTOP;
    sa.sa_handler = cleanup;

    art_printf("before call sigaction\n");
    ...
    if ( ((frag_size == 0) || ((frag_size % 4) != 0)) || (frag_size > 200) ) {
        art_printf("Invalid fragsize, should be multiple of 4 and frag size less than 200\n");
        goto main_exit;
    }
    ...
    if ( initInterface(ar6kifname) ) {
        art_printf("Init interface cfg80211 failed\n");
        cleanup(0);
        return -1;
    }
#if 0
    art_printf("open sock\n");
    sid = sock_init(port);
    if (sid < 0) {
        art_printf("Create socket to ART failed\n");
        cleanup(0);
        return -1;
    }

    if ((recvbytes=sock_recv(cid, line, LINE_ARRAY_SIZE)) < 0) {
        art_printf("Cannot nego packet size\n");
        cleanup(0);
        return -1;
    }

    art_printf("Get nego bytes %d\n", recvbytes);

    if (1 == (*(unsigned int *)line)) {
        reducedARTPacket = 1;
        art_printf("Not supporting reducedARTPacket\n");
        goto main_exit;
    }
    else {
        reducedARTPacket = 0;
    ...
    art_printf("Ready to loop for art packet reduce %d\n", reducedARTPacket);
#endif
    ...
    while (1) {
...
#if 0
        //art_printf("wait for tcp socket\n");
        if ((recvbytes = sock_recv(cid, line, LINE_ARRAY_SIZE)) < 0) {
            art_printf("Cannot recv packet size %d\n", recvbytes);
            cleanup(0);
            return -1;
        }
#endif
        ...
        if (recvbytes < 0)
        {
            art_printf("recvbytes:Cannot recv all packets --Quit!!!!\n");
            cleanup(0);
        ...
        if ( firstpacket == true )
        {
            ...
            art_printf("->FW len %d Command %d recvbytes %d\n",cmdLen,cmdId,recvbytes);
            firstpacket = false;
        }

        //art_printf("Recived bytes from NART %d frag size %d\n",recvbytes,frag_size);
        bufpos = line;
        ...
        while (recvbytes) {
            ...
            //art_printf("Chunk Len %d total size %d respNeeded %d\n",chunkLen,bytesRcvd,resp);
    ...
    */
        ...
        if (sendbytes < 0 || cmdId == DISCONNECT_PIPE_CMD_ID)
        {
            if (cmdId == DISCONNECT_PIPE_CMD_ID)
                art_printf("DISCONNECT_PIPE_CMD_ID --Quit!!!!\n");
            else
                art_printf("Cannot send all packets --Quit!!!!\n");
