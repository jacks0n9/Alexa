...
function do_prepare_image() {
    ...
    # Add Lab126 DNS servers to enable reoslution even when NFS mounted
    ${FAKEROOT} echo "domain corp.lab126.com" > ${TARGET_DIR}/etc/resolv.conf
    ${FAKEROOT} echo "search corp.lab126.com" >> ${TARGET_DIR}/etc/resolv.conf
    ${FAKEROOT} echo "nameserver 172.24.20.150" >> ${TARGET_DIR}/etc/resolv.conf
    ${FAKEROOT} echo "nameserver 10.4.4.10" >> ${TARGET_DIR}/etc/resolv.conf
