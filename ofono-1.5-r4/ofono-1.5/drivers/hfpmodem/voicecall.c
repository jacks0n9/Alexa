				...
				unsigned int value)
{
	...
	case 1:
	{
		...
		/* No call was found! Assumptions in this module have been violated!
		 * Let's get back on the horse by creating one now.
		 */
		if (!notified) {
			call = create_call(vc, 0, 1, CALL_STATUS_ACTIVE, "", 128, 0);
			if (call) {
				ofono_voicecall_notify(vc, call);
			}
		}
