...
#define WD_PTV	0
/* Watchdog WSPR Pending bit */
#define WD_WWPS_WSPR_PENDING	0x10
/* Watchdog WTGR Pending bit */
#define WD_WWPS_WTGR_PENDING	0x08
/* Watchdog WLDR Pending bit */
#define WD_WWPS_WLDR_PENDING	0x04
/* Watchdog WCRR Pending bit */
#define WD_WWPS_WCRR_PENDING	0x02
/* Watchdog WCLR Pending bit */
#define WD_WWPS_WCLR_PENDING	0x01
/* Watchdog trigger pattern */
#define WD_TGR_PATTERN	0x1234
...
static u8 omap3_board_version;

/*******************************************************
 * Routine: get_omap3_board_rev
 * Description: Get the board revision number
 ******************************************************/
...
/*******************************************************
 * Routine: omap3_board_get_revision
 * Description: Read board ID bits for version number
 ******************************************************/
static void omap3_board_get_revision(void)
{
	omap3_board_version = 0;
	...
	omap3_board_version = omap_get_gpio_datain(27)
			| omap_get_gpio_datain(28) << 1 | omap_get_gpio_datain(29) << 2
			| omap_get_gpio_datain(39) << 3 | omap_get_gpio_datain(40) << 4
			| omap_get_gpio_datain(41) << 5 | omap_get_gpio_datain(42) << 6;
...
/*******************************************************
 * Routine: board_is_p
 * Description: Determine if specific P board supported
 ******************************************************/
static int board_is_p(void)
{
	/* Bit-0 is always set for P boards, hence 0x1 */
	return (omap3_board_version & 0x1);
}
...
void config_3430sdram_ddr(void)
{
	...
	/* The timings below are recommended per hw for all 200Mhz memory parts
	 * Trfc=24 ; Trc=11 ; Tras=8 ; Trp=3 ; Trcd=3
	 * Trrd=2 ; Tdpl=3 ; Tdal=6 ; Twtr=2 ; Tcke=2
	 * Txp=2 ; Txsr=24
	 */

	/* SDRC_ACTIM_CTRLA0 register */
	(*(unsigned int*)0x6D00009c) = 0xc2e1b4c6;
	/* SDRC_ACTIM_CTRLB0 register */
	(*(unsigned int*)0x6D0000a0) = 0x00022218;
	/* SDRC_MCFG0 register */
	...
	__raw_writel(SDP_SDRC_EMR_0_DDR, SDRC_EMR_0);
...
static void dpll3_init_36xx(u32 sil_index, u32 clk_index)
{
	...
	/* Getting the base address of Core DPLL param table*/
	if (board_is_p())
		ptr = (dpll_param *) get_36x_core_dpll_param_p();
	else
...
void prcm_init(void)
{
	...
	/* Setup the functional and interface clocks for GPIO,
	 * so we can get board ID.
	 * Appropriate board ID is required to program dpll3 dividers
	 */
	/* Enable GPIO 1 clocks */
	sr32(CM_FCLKEN_WKUP, 3, 1, 0x1);
	sr32(CM_ICLKEN_WKUP, 3, 1, 0x1);
	/* Enable GPIO 3 clocks */
	sr32(CM_FCLKEN_PER, 14, 1, 0x1);
	sr32(CM_ICLKEN_PER, 14, 1, 0x1);
	/* Enable GPIO 2 clocks */
 ...
 ******************************************************/
void wait_for_command_complete(unsigned int wd_base, u32 wait_flag)
{
	int pending = 1;
	do {
		pending = __raw_readl(wd_base + WWPS) & wait_flag;
	} while (pending);
}
...
void watchdog_disable(void)
{
	...
	wait_for_command_complete(WD2_BASE, WD_WWPS_WSPR_PENDING);
...
{
	/* Enable the watchdog, ensure pending writes complete */
	__raw_writel(WD_LOCK1, WD2_BASE + WSPR);
	wait_for_command_complete(WD2_BASE, WD_WWPS_WSPR_PENDING);
...
/****************************************
 * Routine: watchdog_set_timeout
 * Description: Set the watchdog timeout
 *****************************************/
void watchdog_set_timeout(u32 time_in_sec)
{
	static unsigned int wdt_trgr_pattern = WD_TGR_PATTERN;
	u32 timeout = (0xffffffff - ((time_in_sec) * (32768/(1<<WD_PTV))) + 1);
	/* Disable the watchdog */
	watchdog_disable();
	/* Ensure there are not pending writes */
	wait_for_command_complete(WD2_BASE, WD_WWPS_WSPR_PENDING);
	/* Load the timeout value */
	__raw_writel(timeout, WD2_BASE + WLDR);
	/* Ensure pending load is written */
	wait_for_command_complete(WD2_BASE, WD_WWPS_WLDR_PENDING);
	/* Trigger a reload */
	wdt_trgr_pattern = ~wdt_trgr_pattern;
	__raw_writel(wdt_trgr_pattern, (WD2_BASE + WTGR));
	wait_for_command_complete(WD2_BASE, WD_WWPS_WTGR_PENDING);
}
/****************************************
 * Routine: watchdog_init
 * Description: Initialize watchdog clocks
 *****************************************/
void watchdog_init(void)
{
	...
	/* Enable the prescaler, Prescaled by pow(2, PTV) */
	__raw_writel((1 << 5) | (WD_PTV << 2), WD2_BASE + WCLR);
	/* Ensure pending load is written */
	wait_for_command_complete(WD2_BASE, WD_WWPS_WCLR_PENDING);
}
...
#define MUX_DEFAULT()\
	...
	MUX_VAL(CP(GPMC_A6),        (IEN | PTD | DIS | M4)) /*GPMC_A6*/\
	MUX_VAL(CP(GPMC_A7),        (IEN | PTD | DIS | M4)) /*GPMC_A7*/\
	MUX_VAL(CP(GPMC_A8),        (IEN | PTD | DIS | M4)) /*GPMC_A8*/\
	MUX_VAL(CP(GPMC_A9),        (IEN | PTD | DIS | M4)) /*GPMC_A9*/\
	...
	MUX_VAL(CP(DSS_DATA0),      (IEN  | PTD | DIS | M4)) /*GPIO_70 - ADC3101_2 RESET*/\
	MUX_VAL(CP(DSS_DATA2),      (IEN  | PTD | DIS | M4)) /*GPIO_72 - ADC3101_1 RESET*/\
	MUX_VAL(CP(DSS_DATA15),     (IEN  | PTD | DIS | M4)) /*GPIO_85 - AIC3204 RESET*/\
...
int mmc_boot(unsigned char *buf, int dev_num, int part_num) {
	...
	printf("Starting X-loader on mmc-%d...", dev_num);
	...
	// Register the just initialized MMC dev fat partition
	dev_desc = mmc_get_dev(dev_num);
	if (!dev_desc)
		return 0;

	fat_register_device(dev_desc, part_num);
...
void set_bootmode(uint16_t boot_mode)
{
	...
	val = __raw_readl(SCM_BASE + SCM_SCRATCHPAD_OFFSET);
	__raw_writel((val & ~0xFFFF) | boot_mode , SCM_BASE + SCM_SCRATCHPAD_OFFSET);
}
...
/**
 * Time in milli-seconds using 32 Khz clock
 * This may wrap - User beware !!
 */
u_int32_t omap3_get_time_ms()
{
	// Uses 32khz clock to determine time since power up
	u_int32_t val = 0;
	val = __raw_readl(TIMER_32KSYNTIMER_CR);
	// 32 cycles per ms
	return val >> 5;
}
