/***
    ...
    Copyright (c) 2013-2017 Amazon.com, Inc. or its affiliates.
...
PA_MODULE_AUTHOR("Pierre-Louis Bossart, Amazon.com, Inc.");
...
static int sink_input_process_msg_cb(pa_msgobject *obj, int code, void *data, int64_t offset, pa_memchunk *chunk) {
    ...
    switch (code) {
        ...
        case SINK_INPUT_MESSAGE_POST:
            ...
            {
                /* clip the queue to the requested latency if we breach 2x requested latency */
                size_t queue_bytes = pa_memblockq_get_length(u->memblockq);
                size_t desired_bytes = pa_usec_to_bytes(u->latency, &u->sink_input->sample_spec);
                size_t max_bytes = pa_usec_to_bytes(2 * u->latency, &u->sink_input->sample_spec);
                if (queue_bytes > max_bytes) {
                    pa_log_debug("max latency reached; clipping queue");
                    pa_memblockq_drop(u->memblockq, queue_bytes - desired_bytes);
                }
            }
