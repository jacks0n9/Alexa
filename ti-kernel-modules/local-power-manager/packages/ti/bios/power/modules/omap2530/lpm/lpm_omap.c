...
/*
 *  ======== lpm_omap.c ========
 *  LPM driver configured for omap2530 device.
 *
 */
...
// OMAP2530 module and mmr addresses (physical)
#define SYSTEM_CONTROL_ADDR     0x49002000
...
#define PRCM_IVA2_CM_ADDR       0x49006800
...
#define PRCM_IVA2_PRM_ADDR      0x49006800
...
static LPM_Status LPM_off(void *instance)
{
    ...
    int             retry_count = 2;  /* max failure retry count */


    TRACE("lpm.ko: --> LPM_off\n");

top:
    TRACE("lpm.ko: start of sequence\n");

    /* MPU controlled ON + ACTIVE */

    // Turn off automatic clock gating
    ...
    REG(prm + RM_RSTST_IVA2) |= 0x20;
    REG(prm + RM_RSTST_IVA2);

    // release IVA2_RST2 reset
    REG(prm + RM_RSTCTRL_IVA2) &= ~(0x2);
    REG(prm + RM_RSTCTRL_IVA2);

    // wait for IVA2_RST2 signal to be released
    for (i = 0; i < MAX_WAIT_COUNT; i++) {
        if ((REG(prm + RM_RSTST_IVA2) & (0x20))) {
            break;
        }
    }
    if (i == MAX_WAIT_COUNT) {
    ...
    for (i = 0; i < MAX_WAIT_COUNT; i++) {
        if ((REG(cm + CM_IDLEST_IVA2) & 0x1) == 1) {
            break;
        }
    }
    ...
    if ((i == MAX_WAIT_COUNT) && (retry_count > 0)) {
        TRACE("lpm.ko: Error: IVA2 power domain not OFF (1A)\n");
        retry_count--;
        goto top;
    }
    else if (i == MAX_WAIT_COUNT) {
    ...
    // set next IVA2 power state to OFF
    REG(prm + PM_PWSTCTRL_IVA2) |= (0x3 | 1<<18);
    REG(prm + PM_PWSTCTRL_IVA2);

    // ensure the IVA2_SW_RST1 status bit is cleared
    REG(prm + RM_RSTST_IVA2) |= 0x10;
    REG(prm + RM_RSTST_IVA2);
    ...
    if ((i == MAX_WAIT_COUNT) && (retry_count > 0)) {
        TRACE("lpm.ko: Error: IVA2 power domain not OFF (2A)\n");
        retry_count--;
        goto top;
    }
    else if (i == MAX_WAIT_COUNT) {
