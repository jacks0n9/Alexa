...
static long lpm_ioctl(struct file *filp, unsigned int cmd, unsigned long args);
...
static long lpm_ioctl(struct file *filp, unsigned int cmd, unsigned long args)
{
    ...
    if (mutex_lock_interruptible(&inst->mutex)) {
        stat = -ERESTARTSYS;
        goto fail;
    }
...
int lpm_regExtOffFxn(char *device, LPM_ExtPwrFxn fxn, void *arg)
{
    ...
    for (i = 0; i < LPM_DEV_COUNT; i++) {
        if (strcmp(dev->inst[i].name, device) == 0) {
            inst = &dev->inst[i];
            lpm = &inst->lpm;
            break;
        }
    ...
    if (i == LPM_DEV_COUNT) {
        err = -1;
        goto fail;  /* device not found */
    }


    /* enter critical section */
    mutex_lock_interruptible(&inst->mutex);

    /* register the external power off function with the LPM device */
    lpm->command(lpm->instance, LPM_EXTERNAL_VOLTAGE_OFF_FXN, (unsigned long)fxn);
    lpm->command(lpm->instance, LPM_EXTERNAL_VOLTAGE_OFF_ARG, (unsigned long)arg);
...
EXPORT_SYMBOL(lpm_regExtOffFxn);
...
int lpm_regExtOnFxn(char *device, LPM_ExtPwrFxn fxn, void *arg)
{
    ...
    /* Register the external power on function with the LPM device. */
    lpm->command(lpm->instance, LPM_EXTERNAL_VOLTAGE_ON_FXN, (unsigned long)fxn);
    lpm->command(lpm->instance, LPM_EXTERNAL_VOLTAGE_ON_ARG, (unsigned long)arg);
