...
#if defined (OMAP3530)
static inline void armv7_expose_cycle_counter()
{
    ...
    // Enable the cycle counter (with reset)
    asm volatile("mcr p15, 0, %0, c9, c12, 0" :: "r"(0x0000001d)); // 0x01 = enable all counters = 1
                                                                   // 0x02 = 0
                                                                   // 0x04 = reset cycle counter to zero = 1
                                                                   // 0x08 = counts every 64th processor clock cycle = 1
                                                                   // 0x10 = export to ETM = 1
    asm volatile("mcr p15, 0, %0, c9, c12, 1" :: "r"(0x80000000)); // enable cycle counter
    ...
    printk(KERN_INFO "PMNC user-space access is enabled: 0x%x.\n", val);
}
#endif // OMAP3530
...
#if defined (OMAP3530)
    // Initializes the counter and clears it.
    // This must be the only initializer of this timer in the system!
    armv7_expose_cycle_counter();
#endif
